...
function HeroSlide({ item, isActive, className }) {
  ...
  return (
    <div className={cx('hero-slide', { active: isActive })} style={{ backgroundImage: `url(${backgroundPath})` }}>
      <div className={cx('hero-slide-content', { [className]: className })}>
        <div className={cx('hero-slide-content-info')}>
          <h2 className={cx('hero-slide-content-info__title')}>{slideItem.title}</h2>
          <p className={cx('hero-slide-content-info__overview')}>{slideItem.overview}</p>
          <div className={cx('hero-slide-content-info__btns')}>
            <Button onClick={() => navigate(`/movie/${slideItem.id}`)}>Watch now</Button>
